...
(define-minor-mode skt-minor-mode
  ...
  (let* ((modes (skt--list-derived-modes major-mode))
         (tag-vars (mapcar #'tempo-tags-variable modes))
...
;;; Tempo Elements
(defvar tempo-else-key (kbd "C-M-g")
  "Key used to execute else branch in tempo conditional.")

(defun tempo--prompt (prompt)
  "Make prompt for tempo conditional.
PROMPT is preceded with `tempo-else-key'."
  (concat "(" (key-description tempo-else-key) " to quit) " prompt))

(defun tempo-user-elements (element)
